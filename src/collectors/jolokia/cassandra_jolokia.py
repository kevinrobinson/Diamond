...
class CassandraJolokiaCollector(JolokiaCollector):
    ...
    def __init__(self, config, handlers):
        ...
        self.percentiles = map(int, string.split(self.config['percentiles'], ','))
        self.attribute_regex = re.compile(self.config['attribute_regex'])
    ...
    def interpet_bean_with_list(self, prefix, obj):
        ...
        offsets = create_offsets(len(obj))
        for percentile in self.percentiles:
            if not attribute_regex.match(prefix):
                continue
            percentile_value = self.compute_percentile_from_buckets(offsets, buckets, percentile)
            self.collect_bean("%s.p%s" % (prefix, percentile), percentile_value)
    ...
    def compute_percentile(self, offsets, buckets, percentile_int):
        ...
        print "non_zero_points: %s" % non_zero_points
        print "middle_point_index: %s" % middle_point_index

        points_seen = 0
        for index, bucket in enumerate(buckets):
            print "index: %s" % index
            points_seen += bucket
            print "points_seen: %s" % points_seen
